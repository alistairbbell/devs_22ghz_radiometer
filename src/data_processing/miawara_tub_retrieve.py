...
<<<<<<< Updated upstream
#server_basepath = '/instruments/miawara/l2/l2_scaled_hdf_fields'  # Assuming root share access, adjust as needed
server_basepath = '/instruments/miawara/l2/l2_scaled_hdf_fields'  
=======
...
<<<<<<< Updated upstream
output_file_name = 'MIAWARA_concat_H2O_l2_scaled_plus_routine.nc'
=======
...
<<<<<<< Updated upstream
#%%
# Ensure 'concatenated_ds' is sorted by 'time'
concatenated_ds = concatenated_ds.sortby('time')

#%%
# Convert timestamps to datetime objects
timestamps = concatenated_ds['time'].values
#%%
=======
...
import matplotlib.pyplot as plt
import numpy as np
import datetime as dFl

# Plotting
fig, ax = plt.subplots(figsize=(20, 8))
ax.set_ylim(20, 0.010)
ax.set_xlim(dt.datetime(2010, 4, 1), dt.datetime(2023, 11, 1))
ax.set_yscale('log')
ax.set_xlabel('Time', fontsize=20)
ax.set_ylabel('Pressure (hPa)', fontsize=20)

min_val, max_val, diff = 2, 10, 0.01  # Renamed variables to avoid shadowing built-in functions
...
# Dummy data for plotting (replace with actual data)
plotting_data = concatenated_ds.q.values * 1e6
...
timestamps = concatenated_ds['time'].values
time = [dt.datetime.fromtimestamp(ts) for ts in timestamps]            

# Creating contour plot
contour = ax.contourf(time, pressure / 100, plotting_data, levels=np.arange(min_val, max_val + (diff / 2), diff), cmap=cmap)
cbar = plt.colorbar(contour)
cbar.set_label('Mixing Ratio (PPMV)', fontsize=20)

# Set colorbar ticks
ticks_loc = np.arange(min_val, max_val + 0.01, (max_val - min_val) / 10)
cbar.set_ticks(ticks_loc)
cbar.ax.tick_params(labelsize=20)

# Add grey to no data points
for i in range(1, len(time)):
    gap = (time[i] - time[i-1]).days
    if gap > 3:
        print(time[i])
